...
const plannerHighlights: LandingCardItem[] = [
  { icon: CalendarCheck, title: "Itinerario", text: "Mañana, tarde y noche ordenadas por tiempo real." },
  { icon: Map, title: "Mapa", text: "Lugares, distancias y rutas en una sola vista." },
  { icon: Heart, title: "Favoritos", text: "Guarda planes y vuelve cuando estés cerca." },
  { icon: Ticket, title: "Promos", text: "Ofertas útiles en el momento correcto.", feature: "promotions" },
];
...
const dayPlan: LandingPlanItem[] = [
  { time: "9:30 AM", title: "Café con sombra", text: "Cerca del hotel, buenas reseñas y ambiente tranquilo.", icon: Search },
  { time: "11:00 AM", title: "Audiotour histórico", text: "Tres paradas narradas antes de que suba el calor.", icon: Volume2, feature: "audioTours" },
  { time: "1:20 PM", title: "Almuerzo local", text: "Promoción activa y ruta corta desde la parada anterior.", icon: Ticket, feature: "promotions" },
  { time: "4:45 PM", title: "Spot para fotos", text: "Atardecer recomendado según ubicación y clima del momento.", icon: Camera },
];

const compareRows = [
  ["Antes", "12 pestañas abiertas", "Sin saber qué queda cerca", "Promos que aparecen tarde"],
  ["Con Xitty", "Un plan vivo", "Mapa + tiempos + favoritos", "Acciones listas para reservar"],
];

const modes = [
  { label: "Pareja", text: "Planes caminables, reservas y atardeceres.", icon: UsersRound },
  { label: "Familia", text: "Rutas cortas, lugares seguros y pausas claras.", icon: ShieldCheck },
  { label: "Negocios", text: "Comidas cerca, horarios y traslados rápidos.", icon: Clock3 },
  { label: "Nómada", text: "Cafés, Wi-Fi, cultura y barrios con ambiente.", icon: Smartphone },
];

const trustSignals = [
  { icon: BadgeCheck, text: "Curaduría Xitty y recomendaciones locales" },
  { icon: Languages, text: "Base multi-idioma para visitantes" },
  { icon: LocateFixed, text: "Contexto por ubicación cuando el usuario lo permite" },
...
export default function LandingPage() {
  const activePlannerHighlights = plannerHighlights.filter(isLandingItemVisible);
  ...
  return (
    <main className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
        ...
        <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 pb-12 pt-8 sm:px-6 lg:grid-cols-[minmax(0,600px)_minmax(380px,1fr)] lg:px-8 lg:pb-16 lg:pt-20">
          <div className="flex flex-col items-start self-center">
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-[var(--text-muted)] sm:text-lg">
              Xitty junta {heroCapabilities} para que no tengas que
              improvisar como turista perdido.
            </p>
            <div className="mt-7 flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
              <Link
                href="/register"
                className={cn(landingButton, landingButtonVariants.primary, "w-full sm:w-auto")}
              >
                Empezar a planear
              ...
              {featureFlags.promotions ? (
                <Link
                  href="/promotions"
                  className={cn(landingButton, landingButtonVariants.soft, "w-full sm:w-auto")}
                >
                  Ver promos activas
                </Link>
              ) : null}
      ...
      {landingSectionFlags.landingPlanner ? (
      <section id="planner" className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-7 grid gap-3">
              {compareRows.map(([label, one, two, three]) => (
                <div key={label} className="rounded-lg bg-[var(--surface)] p-4 shadow-[var(--shadow-1)]">
      ...
      {landingSectionFlags.landingFeatures ? (
      <section id="features" className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          ...
          <p className="max-w-md text-base leading-7 text-[var(--text-muted)]">
            Xitty lo ordena alrededor de la decisión: qué vale la pena, cómo llegar,
            cuánto cuesta y qué puedes guardar.
          </p>
        </div>

        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {activeReplacementFeatures.map(({ icon: Icon, title, text }) => (
            <article key={title} className="rounded-lg bg-[var(--surface)] p-5 shadow-[var(--shadow-1)]">
      ...
      {showAudioTours ? (
      <section id="audiotours" className="bg-[var(--ink)] text-[var(--text-inverse)]">
        <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8 lg:py-16">
          ...
          <div className="flex flex-col justify-center">
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-white/78">
              Los audiotours convierten lugares de interés en recorridos narrados
              por paradas.
              {featureFlags.rewardsRally
            ...
            <div className="mt-7 grid gap-3">
              {audioTourHighlights.map(([Icon, title, text]) => (
                <article key={String(title)} className="rounded-lg bg-white/9 p-4">
                  <div className="flex gap-4">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-pill bg-white text-[var(--ink)]">
                      <Icon className="h-5 w-5" aria-hidden="true" />
                    </span>
                    <div>
                      <h3 className="font-semibold tracking-normal">{title as string}</h3>
        ...
        <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[0.85fr_1.15fr] lg:px-8 lg:py-16">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {modes.map(({ label, text, icon: Icon }) => (
              <article key={label} className="rounded-lg bg-[var(--surface)] p-5 shadow-[var(--shadow-1)]">
                <span className="flex h-10 w-10 items-center justify-center rounded-pill bg-[var(--accent-soft)] text-[var(--accent)]">
                  <Icon className="h-5 w-5" aria-hidden="true" />
                </span>
                <h3 className="mt-5 text-lg font-semibold tracking-normal">{label}</h3>
                <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">{text}</p>
              </article>
            ))}
          </div>
      ...
      {landingSectionFlags.landingTrust ? (
      <section id="confianza" className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {trustSignals.map(({ icon: Icon, text }) => (
              <article key={text} className="flex gap-3 rounded-lg bg-[var(--surface)] p-4 shadow-[var(--shadow-1)]">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-pill bg-[var(--accent-soft)] text-[var(--accent)]">
                  <Icon className="h-5 w-5" aria-hidden="true" />
                </span>
                <p className="self-center text-sm font-semibold leading-6 text-[var(--text)]">{text}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="border-y border-[var(--border)] bg-[var(--bg-subtle)]">
        <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:py-16">
          ...
          <div className="flex flex-col justify-center">
            ...
            <h2 className="max-w-2xl text-3xl font-semibold leading-tight tracking-normal sm:text-5xl">
              Las promos, reservas y micrositios aparecen donde se toma la decisión.
            </h2>
            <p className="mt-4 max-w-xl text-base leading-7 text-[var(--text-muted)]">
              El negocio no queda enterrado en un catálogo. Entra en el momento
              exacto del plan: cerca, útil y con acción clara.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-8 lg:py-16">
        ...
        <div className="grid gap-3">
          {faqs.map(({ question, answer }) => (
            <details key={question} className="group rounded-lg bg-[var(--surface)] p-5 shadow-[var(--shadow-1)]">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-base font-semibold tracking-normal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-4 focus-visible:ring-offset-[var(--bg)]">
                {question}
                <ArrowRight className="h-4 w-4 shrink-0 text-[var(--accent)] transition-transform duration-150 group-open:rotate-90 motion-reduce:transition-none" aria-hidden="true" />
              </summary>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--text-muted)]">{answer}</p>
            </details>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pb-12 sm:px-6 lg:px-8 lg:pb-16">
        <div className="relative isolate overflow-hidden rounded-lg bg-[var(--surface-mint)] p-6 shadow-[var(--shadow-flat)] sm:p-8 lg:p-10">
          ...
          />
          <div aria-hidden="true" className="absolute inset-0 -z-10 bg-[linear-gradient(90deg,var(--surface-mint)_0%,rgba(236,250,236,0.94)_46%,rgba(236,250,236,0.62)_100%)]" />
          <div className="max-w-3xl">
            ...
            <div className="mt-7 flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
              <Link href="/register" className={cn(landingButton, landingButtonVariants.primary, "w-full sm:w-auto")}>
                Empezar ahora
                <ArrowRight className="h-4 w-4" aria-hidden="true" />
              </Link>
              <Link href="/login" className={cn(landingButton, landingButtonVariants.secondary, "w-full sm:w-auto")}>
                Ya tengo cuenta
              </Link>
            </div>
          </div>
...
function NavLink({ href, children }: { href: string; children: ReactNode }) {
  return (
    <Link href={href} className="text-sm font-semibold text-[var(--text-muted)] underline-offset-4 hover:text-[var(--text)] hover:underline">
      {children}
    </Link>
  );
}

function LandingPill({ icon: Icon, children }: { icon: LucideIcon; children: ReactNode }) {
  return (
...
function HeroPlannerMockup({ items }: { items: LandingPlanItem[] }) {
  return (
    <div className="relative hidden min-h-[560px] self-end lg:block" aria-label="Vista previa de planner Xitty">
      <div className="absolute right-0 top-2 w-[430px] rounded-lg border border-[var(--border-strong)] bg-[var(--surface)] p-4 shadow-[var(--shadow-flat)]">
        <div className="flex items-center justify-between gap-3 border-b border-[var(--border)] pb-3">
          <div>
            <p className="text-xs font-semibold text-[var(--text-muted)]">Mi viaje</p>
            <h2 className="text-xl font-semibold tracking-normal">Barranquilla, 3 días</h2>
          </div>
          <span className="rounded-pill bg-[var(--accent)] px-3 py-1 text-xs font-semibold text-[var(--accent-fg)]">Listo</span>
        </div>
        <div className="mt-4 grid gap-3">
          {items.slice(0, 3).map(({ time, title, text, icon: Icon }) => (
            <div key={title} className="flex gap-3 rounded-lg bg-[var(--bg-subtle)] p-3">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-pill bg-[var(--accent-soft)] text-[var(--accent)]">
                <Icon className="h-5 w-5" aria-hidden="true" />
              </span>
              <div>
                <p className="text-xs font-semibold text-[var(--text-soft)]">{time}</p>
      ...
      <div className="absolute bottom-4 left-0 w-[360px] rounded-lg border border-[var(--border-strong)] bg-[var(--surface)] p-4 shadow-[var(--shadow-flat)]">
        <div className="relative h-56 overflow-hidden rounded-lg bg-[var(--surface-mint)]">
          <Image src="/landing/xitty-cartoon-river-route.png" alt="Mapa ilustrado de ruta Xitty" fill sizes="360px" className="h-full w-full object-cover" />
          <div className="absolute left-4 top-4 rounded-pill bg-[var(--surface)] px-3 py-1 text-xs font-semibold text-[var(--text)] shadow-[var(--shadow-1)]">Mapa vivo</div>
          <div className="absolute bottom-4 right-4 rounded-lg bg-[var(--surface)] p-3 shadow-[var(--shadow-1)]">
            <p className="text-xs font-semibold text-[var(--text-muted)]">Siguiente parada</p>
            <p className="text-sm font-semibold">
              {featureFlags.audioTours ? "Audiotour histórico" : "Ruta recomendada"}
            </p>
          </div>
        </div>
      </div>
      {featureFlags.aiChat ? (
        <div className="absolute bottom-20 right-12 w-52 rounded-lg bg-[var(--ink)] p-4 text-[var(--text-inverse)] shadow-[var(--shadow-flat)]">
          <div className="flex items-center gap-2 text-sm font-semibold">
            <Sparkles className="h-4 w-4 text-[var(--secondary)]" aria-hidden="true" />
            Xi sugiere
          </div>
  ...
  return (
    <div className="rounded-lg border border-[var(--border-strong)] bg-[var(--surface)] p-4 shadow-[var(--shadow-flat)]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] pb-4">
        <div>
          <p className="text-xs font-semibold text-[var(--text-muted)]">Plan recomendado</p>
          <h3 className="text-2xl font-semibold tracking-normal">Hoy: cultura + comida + atardecer</h3>
        </div>
      ...
      <div className="mt-4 grid gap-3">
        {items.map(({ time, title, text, icon: Icon }) => (
          <article key={title} className="flex gap-4 rounded-lg bg-[var(--bg-subtle)] p-4">
            <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-pill bg-[var(--accent-soft)] text-[var(--accent)]">
              <Icon className="h-5 w-5" aria-hidden="true" />
            </span>
            <div>
              <p className="text-xs font-semibold text-[var(--text-soft)]">{time}</p>
              <h4 className="mt-1 text-base font-semibold tracking-normal">{title}</h4>
...
function LandingImageCard({ src, alt, label, text, icon: Icon }: { src: string; alt: string; label: string; text: string; icon: LucideIcon }) {
  return (
